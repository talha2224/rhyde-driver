...
const Credit = () => {
  ...
  const fetchWallet = async (uid) => {
    try {
      const res = await axios.get(`${config.baseUrl}/wallet/history/driver/${uid}`);
  ...
  const fetchTransactions = async (uid) => {
    try {
      const res = await axios.get(`${config.baseUrl}/transaction?role=driver&userId=${uid}`);
        ...
        <View style={styles.previousPurchasesSection}>
          ...
          {transactions.map((item, index) => (
            <View key={index} style={styles.purchaseItem}>
              ...
              <Text style={styles.purchaseExtra}>{item.message}</Text>
            </View>
          ))}
        </View>
      </ScrollView>
